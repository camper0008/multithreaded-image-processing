...
fn multi_threaded() -> u128 {
    let now = Instant::now();
    ["1.webp", "2.webp", "3.webp"].par_iter().for_each(|i| {
        let input_path = format!("input/{i}");
        ...
        let output_path = format!("output-multi/{i}");
...
fn single_threaded() -> u128 {
    let now = Instant::now();
    ["1.webp", "2.webp", "3.webp"].iter().for_each(|i| {
        let input_path = format!("input/{i}");
        let mut image = open(input_path).unwrap().into_rgb8();
        let (width, height) = image.dimensions();
        (0..width).for_each(|x| {
        ...
        let output_path = format!("output-single/{i}");
        image.save(output_path).unwrap();
    });
    now.elapsed().as_millis()
}
...
fn main() {
    ...
    println!("time for single threaded: {multi_threaded_time}ms");
}
